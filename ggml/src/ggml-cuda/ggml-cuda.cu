...
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
    // cudaMemGetInfo lazily creates a primary context; release it afterward if it wasn't already active.
    CUdevice device;
    CU_CHECK(cuDeviceGet(&device, ctx->device));

    unsigned int primary_ctx_flags;
    int primary_ctx_active;
    CU_CHECK(cuDevicePrimaryCtxGetState(device, &primary_ctx_flags, &primary_ctx_active));
#endif
